...
public class MemberController {
	...
	public String myPageLeave(HttpServletRequest request, Model model) {
		...
		command.execute(sqlSession, model);
		/*
		 * // 1. 전달 받은 파라미터 저장 String mId = request.getParameter("mId");
		 * 
		 * // 2. mId 회원 삭제 HotelFiveDAO hDAO = sqlSession.getMapper(HotelFiveDAO.class);
		 * int result = hDAO.leaveMember(mId);
		 * 
		 * // 3. 결과를 저장할 JSONObject 생성 JSONObject obj = new JSONObject();
		 * 
		 * // 4. JSONObject 에 결과 result 저장 if (result > 0) { obj.put("result",
		 * "SUCCESS"); request.getSession().invalidate(); // 세션 초기화 } else {
		 * obj.put("result", "FAIL"); }
		 */
	...
	public String goAdmission(HttpServletRequest request, Model model) {
	...
	/*
	 * @RequestMapping("noticeBoardInsertPage") public String
	 * goAdminPostinsert(HttpServletRequest request, Model model) {
	 * 
	 * model.addAttribute("request", request); command = new AdminPostListCommand();
	 * command.execute(sqlSession, model);
	 * 
